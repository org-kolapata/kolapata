...
class HrPayslip(models.Model):
    ...
    def compute_sheet(self):
        ...
        payslip lines"""
        for rec in self:
            salary_line = rec.struct_id.rule_ids.filtered(
                            lambda x: x.code == 'SAR')
            if salary_line:
                ...
                print("Last advance amount: ", get_amount)

                if get_amount:
                    if rec.date_from <= get_amount.date <= rec.date_to:
                        amount = get_amount.advance
                        name = salary_line.id
                        code = salary_line.code
                        if (code not in rec.input_line_ids.mapped('input_type_id').
                                mapped('code')):
                            rec.input_data_salary_line(name, amount)
